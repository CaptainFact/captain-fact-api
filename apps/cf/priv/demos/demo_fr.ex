defmodule CF.DemoFr do
  @moduledoc"""
  Run a demo in real-time. To use it from iex console:

      iex> List.first(c(Path.join(:code.priv_dir(:cf), "demos/demo_fr.ex"))).init_and_run
  """

  import DB.Factory
  import CF.Actions.ActionCreator, only: [action_add: 3, action_create: 3]
  ...
  defp add_comment(users, video_id, statement_id, comment_base, reply_to_id \\ nil) do
    ...
    comment =
      ...
      |> Comments.add_comment(video_id, params, comment_base[:source], fn comment ->
        ...
        CF.Web.Endpoint.broadcast(
          "comments:video:#{VideoHashId.encode(video_id)}", "comment_updated",
          CF.Web.CommentView.render("comment.json", comment: comment)
        )
      end)
