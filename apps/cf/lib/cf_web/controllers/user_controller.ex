defmodule CF.Web.UserController do
  ...
  def create(conn, params = %{"user" => user_params}) do
    case Accounts.create_account(user_params, Map.get(params, "invitation_token")) do
      ...
      {:error, message} ->
        conn
        |> put_status(:bad_request)
        |> render(CF.Web.ErrorView, "error.json", %{message: message})
    end
  end
  ...
  def update(conn, params) do
    ...
    |> case do
      ...
      {:error, changeset} ->
        ...
        |> render(CF.Web.ChangesetView, :error, changeset: changeset)
    end
  end
  ...
  def complete_onboarding_step(conn, %{"step" => step}) do
    ...
    |> Result.either(
      fn reason ->
        conn
        |> put_status(:unprocessable_entity)
        |> render(CF.Web.ChangesetView, "error.json", %{changeset: reason})
      end,
      fn user ->
        conn
