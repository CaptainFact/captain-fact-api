defmodule CaptainFact.Sources.FetcherTest do
  ...
  test "Fetch everything even if some fails or crash" do
    ...
    for url <- valid_urls ++ crashing,
        do: Bypass.expect(bypass, "GET", url, plug_response(200, @valid_attributes))

    for url <- invalid_urls,
        do: Bypass.expect(bypass, "GET", url, plug_response(500, @valid_attributes))
