defmodule CaptainFact.AuthenticatorTest do
  ...
  describe "Identity" do
    test "can login" do
      ...
      authenticated_user =
        Authenticator.get_user_for_email_password(user.email, password)

      assert user.id == authenticated_user.id
    end

    property "password must be correct" do
      password = "IfPropertyTestingFailsWithThisString,itIsNotABugButAVeryVeryRareCase,iMeanAlmostImpossible!!!"
      user = insert_user_with_custom_password(password)
