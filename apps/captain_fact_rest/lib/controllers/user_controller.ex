  ...
  def create(conn, params = %{"user" => user_params}) do
    case Accounts.create_account(user_params, Map.get(params, "invitation_token")) do
      ...
      {:error, message} ->
        conn
        |> put_status(:bad_request)
        |> render(CaptainFactREST.ErrorView, "error.json", %{message: message})
    end
  end
  ...
  def update(conn, params) do
    ...
    |> case do
      ...
      {:error, changeset} ->
        ...
        |> render(CaptainFactREST.ChangesetView, :error, changeset: changeset)
    end
  end
