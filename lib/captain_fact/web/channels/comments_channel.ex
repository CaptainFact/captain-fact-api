defmodule CaptainFact.Web.CommentsChannel do
  use CaptainFact.Web, :channel

  import CaptainFact.Web.UserSocket, only: [rescue_channel_errors: 1]
  alias CaptainFact.Web.{ Comment, CommentView, User, Vote, VoteView, Flag, Source }
  ...
  defp fetch_source_metadata_and_update_comment(comment = %Comment{source: source = %{title: nil, url: url}}, topic) do
    case fetch_source_metadata(url) do
      ...
      {:ok, source_params} ->
        ...
        CaptainFact.Web.Endpoint.broadcast(topic, "comment_updated", rendered_comment)
    end
  end
